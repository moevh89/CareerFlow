...
3. **Lokale Datenbank initialisieren (SQLite)**
   Da der webbasierte Setup-Prozess (`setup.php`) nur für MariaDB/MySQL ausgelegt ist, muss die lokale SQLite-Datenbank manuell über das CLI initialisiert werden.
   Lege dazu den Daten-Ordner an und führe die Migrationen aus:

   ```bash
   mkdir -p data
   php -r "require 'src/Core/Dotenv.php'; require 'src/Core/Database.php'; require 'src/Core/Migrator.php'; App\Core\Dotenv::load('.env'); App\Core\Migrator::migrate(); echo 'Migration erfolgreich!';";
   ```
...
5. **App aufrufen**
   Öffne `http://localhost:8000` im Browser. Da die SQLite-Datenbank nun initialisiert ist, kommst du direkt zum Login.
