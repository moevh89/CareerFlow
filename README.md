...
2. **Umgebungsvariablen konfigurieren**
   ...
   *Hinweis:* Standardmäßig ist in der `.env.example` SQLite als Treiber voreingestellt (`DB_DRIVER=sqlite`). Die Datenbankdatei wird dann im Ordner `data/` erwartet.

3. **Datenbank initialisieren (SQLite)**
   Da der webbasierte Setup-Prozess nur MySQL/MariaDB unterstützt, musst du die SQLite-Datenbank für die lokale Entwicklung manuell initialisieren:
...
5. **App aufrufen**
   Öffne `http://localhost:8000` im Browser. Die App ist nun einsatzbereit.
